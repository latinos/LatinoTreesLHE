...
namespace LHEF {
 ...
 public:
   ...
   * Copy-constructor.
   */
  HEPRUP(const HEPRUP & x) : IDBMUP(x.IDBMUP), EBMUP(x.EBMUP), PDFGUP(x.PDFGUP), PDFSUP(x.PDFSUP), IDWTUP(x.IDWTUP), NPRUP(x.NPRUP), XSECUP(x.XSECUP), XERRUP(x.XERRUP), XMAXUP(x.XMAXUP), LPRUP(x.LPRUP) {}
 ...
 public:
   ...
   * Copy-constructor.
   */
  HEPEUP(const HEPEUP & x) : NUP(x.NUP), IDPRUP(x.IDPRUP), XWGTUP(x.XWGTUP), XPDWUP(x.XPDWUP), SCALUP(x.SCALUP), AQEDUP(x.AQEDUP), AQCDUP(x.AQCDUP), IDUP(x.IDUP), ISTUP(x.ISTUP), MOTHUP(x.MOTHUP), ICOLUP(x.ICOLUP), PUP(x.PUP), VTIMUP(x.VTIMUP), SPINUP(x.SPINUP) {}
 ...
 public:
   ...
   */
  Reader(std::string filename) : intstream(filename.c_str()), file(intstream) {
   init();
  }
